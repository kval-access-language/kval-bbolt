...
//Create bucket and key/value entries in bbolt from a kval structure
...
//Retrieve an entry from a bbolt db from a kval structure
...
//Retrieve an entry from a bbolt db using regular expression
...
//Delete a single bucket from a bbolt db from a KVAL structure
...
//Delete all the keys in a bbolt bucket leaving Bucket in tact
...
//Delete a key and its corresponding value from a bbolt db
...
//Rename a bucket (full OR empty) in a bbolt db
...
//Rename a key in a bbolt db based on described KVAL capabilities such as rename
...
//Check to see if a key exists in a bbolt bucket, per KVAL LIS capabilities
...
//Retrieve a bucket pointer to work with from the bbolt db
