...
const (
	...
	opcodeRoot                    // we're working with Root bucket, e.g. can't stat Root/bbolt.Tx
	opcodeUnknown                 // cannot stat for an unknown function
)

//get bbolt bucket stats
...
// statdb allows kvalresult structures to be annotated with bbolt bucket stats
func statdb(kb Kvalboltdb, kr Kvalresult) (Kvalresult, error) {
	...
	switch kr.opcode {
	case opcodeRoot:
		// cannot stat bbolt.Tx/Root bucket
		kr.Stats = bolt.BucketStats{}
		return kr, nil
	case opcodeNormal:
